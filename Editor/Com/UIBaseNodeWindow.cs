...
public class UIBaseNodeWindow : EditorWindow
{
    ...
    {
        _target = Selection.activeGameObject.GetComponent<UIBaseNode>();
    ...
    public void FindBack()
    {
        try
        {
            ...
            if(GUILayout.Button("查找节点"))
            {
                ...
                {
                    if (item.tag.ToLower() == strIpt.ToLower() && item.transform != null)
                    {
                        isFound = true;
                ...
                if (!isFound)
                {
                    foreach (var item in _target.nodes)
                    {
                        var name = item.transform.gameObject.name;
                        if (name.ToLower() == strIpt.ToLower() && item.transform != null)
                        {
                            isFound = true;
                            Selection.activeGameObject = item.transform.gameObject;
                        }
