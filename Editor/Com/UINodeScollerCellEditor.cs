...
[CustomEditor(typeof(UINodeScollerCell))]
public class UINodeScollerCellEditor : Editor
{
    UINodeScollerCell _target;

    ReorderableList nodeList;
    void OnEnable()
    {
        _target = target as UINodeScollerCell;
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("更新脚本"))
        {
            ...
            UINodeScollerCell uINodeScollerCell = Selection.activeGameObject.GetComponent<UINodeScollerCell>();
            if (uIPanel == null)
            {
                ZLogUtil.LogError("没有UINodePanel");
            }
            string path = PathFinderEditor.GetPrefabAssetPath(uIPanel.gameObject, out panelName);
            ScriptCreater.CreateScrollerCellClassName(path,panelName, uINodeScollerCell.transform.name, uINodeScollerCell);
        }
